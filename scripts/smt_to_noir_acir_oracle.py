#!/usr/bin/env python3
"""
Extends smt_to_noir_bb_proofs.py with an ACIR SMT oracle step.

For each compiled circuit the ACIR constraints (BLACKBOX::RANGE + ASSERT only,
BRILLIG is skipped) are translated to QF_NIA SMT with arithmetic mod the BN254
prime and checked with Z3. The oracle runs once per formula (on idx==0).

Bug oracles (in addition to those from smt_to_noir_bb_proofs):
  UNSAT formula + ACIR oracle SAT   → errors/acir_oracle_violation/  (circuit weaker than SMT)
  SAT formula   + ACIR oracle UNSAT → errors/acir_oracle_violation/  (circuit over-constrained)
  SAT+overflow  + ACIR oracle SAT   → logged as false-positive overflow

New CLI flags:
  --acir-oracle-timeout SECS   Z3 timeout for the ACIR oracle (default: 60)

Usage: same as smt_to_noir_bb_proofs.py, plus the new flag.
"""
...
from smt_to_noir_bb_proofs import (
    bb_pipeline,
    run_bb_write_vk,
    run_bb_prove,
    run_bb_verify,
    save_bb_error,
    _write_recent_stats as _bb_write_recent_stats,
    BB_PATH,
    _STATS_INTERVAL,
)
...
from acir_inspector import parse_acir
from acir_smt_oracle import build_smt
...
def run_acir_oracle(project_dir: Path, timeout: int) -> str:
    """
    Compile ACIR for the Nargo project at project_dir, translate to QF_NIA SMT
    (RANGE + ASSERT only, mod BN254), and run Z3.
    Returns 'sat', 'unsat', 'unknown', or 'timeout'.
    """
    try:
        result = subprocess.run(
            ["nargo", "info", "--print-acir", "--silence-warnings"],
            cwd=project_dir,
            capture_output=True,
            text=True,
            timeout=120,
    ...
    try:
        ...
        verdict = (z3.stdout + z3.stderr).strip().split("\n")[0].strip()
        return verdict if verdict in ("sat", "unsat") else "unknown"
...
def save_acir_violation(
    ...
    dest = errors_dir / "acir_oracle_violation" / folder_name
    ...
    (tmp / "acir_oracle_note.txt").write_text(note)
    if dest.exists():
        shutil.rmtree(dest)
    tmp.rename(dest)
...
# ---------------------------------------------------------------------------
# Stats writer (extends bb_proofs version)
# ---------------------------------------------------------------------------
...
def _write_recent_stats(path: Path, stats: dict, start: int, end: int) -> None:
    lines = [
        ...
        f"acir_oracle   : {stats.get('acir_oracle_sat',0)} sat  |  {stats.get('acir_oracle_unsat',0)} unsat  |  {stats.get('acir_oracle_violation',0)} violation  |  {stats.get('acir_oracle_timeout',0)} timeout",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="SMT-LIB → Noir + bb proofs + ACIR SMT oracle."
    )
    ...
    parser.add_argument("--witness-workers", type=int, default=4, metavar="N")
    parser.add_argument("--bb-workers", type=int, default=2, metavar="M")
    parser.add_argument("--bb-timeout", type=int, default=300, metavar="SECS")
    parser.add_argument(
        "--acir-oracle-timeout", type=int, default=60, metavar="SECS",
        help="Z3 timeout for the ACIR SMT oracle (default: 60).",
    )
    ...
    print(f"Workers: {args.witness_workers} witness / {args.bb_workers} bb  |  acir oracle timeout: {args.acir_oracle_timeout}s")
    ...
    def process_formula(stem: str, smt_content: str, bb_pool: ThreadPoolExecutor) -> None:
        ...
        # 3. Find satisfying witness
        z3_failed_error = None
        ...
        if not models:
            ...
            for idx in range(args.max_witnesses):
                ...
                # ACIR oracle — run once per formula; ACIR is independent of Prover.toml
                if idx == 0:
                    acir_verdict = run_acir_oracle(project_dir, args.acir_oracle_timeout)
                    locked_print(f"  [{folder_name}] acir oracle     ... {acir_verdict.upper()}")
                    if acir_verdict == "sat":
                        # SMT UNSAT but ACIR SAT — satisfiability not preserved
                        inc("acir_oracle_violation")
                        save_acir_violation(
                            errors_dir, folder_name,
                            noir_source, boundary_toml, pkg_name,
                            smt_content, smt_filename,
                            "ACIR ORACLE VIOLATION: SMT UNSAT but ACIR oracle SAT\n",
                            safe_math_src, _z3_unsat_query,
                        )
                    elif acir_verdict == "unsat":
                        inc("acir_oracle_unsat")
                    elif acir_verdict == "timeout":
                        inc("acir_oracle_timeout")
                    else:
                        inc("acir_oracle_unknown")
            return
        ...
        # SAT path
        # -----------------------------------------------------------------------
        acir_oracle_done = False  # run oracle only once per formula
        ...
        for idx, model in enumerate(models):
            ...
            # ACIR oracle — once per formula; satisfiability must match SMT
            if not acir_oracle_done:
                acir_oracle_done = True
                acir_verdict = run_acir_oracle(project_dir, args.acir_oracle_timeout)
                locked_print(f"  [{folder_name}] acir oracle     ... {acir_verdict.upper()}")
                if acir_verdict == "unsat":
                    # SMT SAT but ACIR UNSAT — satisfiability not preserved
                    inc("acir_oracle_violation")
                    save_acir_violation(
                        errors_dir, folder_name,
                        noir_source, prover_toml, pkg_name,
                        smt_content, smt_filename,
                        "ACIR ORACLE VIOLATION: SMT SAT but ACIR oracle UNSAT\n",
                        safe_math_src, _z3_sat_query,
                    )
                elif acir_verdict == "sat":
                    inc("acir_oracle_sat")
                elif acir_verdict == "timeout":
                    inc("acir_oracle_timeout")
                else:
    ...
    with ThreadPoolExecutor(max_workers=args.bb_workers) as bb_pool:
        with ThreadPoolExecutor(max_workers=args.witness_workers) as witness_pool:
            ...
            for stem, smt_content in _iter_formulas():
                pending.append(witness_pool.submit(process_formula, stem, smt_content, bb_pool))
            _n_done = 0
            for f in as_completed(pending):
                ...
                _n_done += 1
                if _n_done % _STATS_INTERVAL == 0:
                    with stats_lock:
                        _snap = dict(counters)
                    _delta = {k: _snap.get(k, 0) - _window_snap.get(k, 0) for k in _snap}
                    _write_recent_stats(recent_stats_path, _delta, _window_start, _n_done)
                    _window_snap = _snap
                    _window_start = _n_done + 1

    c = counters
    total_bugs = (c.get("sat_error", 0) + c.get("unsat_error", 0)
                  + c.get("oracle_bug", 0) + c.get("bb_verify_bug", 0)
                  + c.get("acir_oracle_violation", 0))
    print(f"\n{'='*60}")
    ...
    print(f"acir_oracle   : {c.get('acir_oracle_sat',0)} sat  |  {c.get('acir_oracle_unsat',0)} unsat  |  {c.get('acir_oracle_violation',0)} mismatch  |  {c.get('acir_oracle_timeout',0)} timeout")
    print(f"timeout       : {c.get('n_timeout',0)}")
    print(f"{'─'*40}")
    print(f"total bugs    : {total_bugs}  (overflows not counted unless acir mismatch)")
